    ...
    class Solution {
        public int isPrefixOfWord(String sentence, String searchWord) {
            ...
            while (sentenceIndex < sentence.length()) {
                //出现空，重新计算,结果加1
                if (sentence.charAt(sentenceIndex) == ' ') {
                    result++;
                    sentenceIndex++;
                    searchWordIndex = 0;
                    continue;
                }
                if (sentence.charAt(sentenceIndex) == searchWord.charAt(searchWordIndex)) {
                    //相等继续判断
                    sentenceIndex++;
                    searchWordIndex++;
                    ...
                    while (sentenceIndex < sentence.length() && sentence.charAt(sentenceIndex) != ' ') {
                        sentenceIndex++;
                    }
                }
            }
